...
## writing own methods for generating embedding
class strubert_embd(nn.Module):

    def __init__(self, tabert_path='/path/to/tabert/model.bin',device='cuda:0'):
        """"Constructor of the class."""
        super(strubert_embd, self).__init__()
    ...
    def forward(self, tables1, metas1, tables2, metas2):
        ...
        return context_encoding1, column_encoding1, context_encoding_row1, row_encoding1
    

    def get_similarity(self, context_encoding1, column_encoding1, context_encoding_row1, row_encoding1, context_encoding2, column_encoding2, context_encoding_row2, row_encoding2):
    ...
    def calculate_similarity(self, context_encoding1, column_encoding1, context_encoding_row1, row_encoding1):
        column_guided_cls = context_encoding1[:, 0]
        row_guided_cls=context_encoding_row1[:, 0]

        batch_size = row_encoding1.shape[0]
        num_rows1 = row_encoding1.shape[1]
        num_cols1 = column_encoding1.shape[1]
        ...
        seqa = torch.zeros([batch_size, num_cols1 + 2], dtype=torch.long).to(self.device)
        col_seq = torch.cat([cls_to_add, column_encoding1, sep_to_add], dim=1)
        col_types = seqa
        col_emb = self.ranking_embedding(col_seq, col_types)
        ...
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        encoded_layers, _ = self.encoder(col_emb, extended_attention_mask,output_all_encoded_layers=False)
        sequence_output = encoded_layers[-1]
        pooled_output_col = self.pooler(sequence_output)


        seqa = torch.zeros([batch_size, num_rows1 + 2], dtype=torch.long).to(self.device)
        row_seq = torch.cat([cls_r_to_add, row_encoding1, sep_to_add], dim=1)
        row_types = seqa
        row_emb = self.ranking_embedding(row_seq, row_types)
        ...
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        encoded_layers, _ = self.encoder(row_emb,extended_attention_mask,output_all_encoded_layers=False)
        sequence_output = encoded_layers[-1]
        pooled_output_row = self.pooler(sequence_output)
    ...
    def get_similarity_custom(self, context_encoding1, column_encoding1, context_encoding_row1, row_encoding1, context_encoding2, column_encoding2, context_encoding_row2, row_encoding2):
        
        res1 = self.calculate_similarity(context_encoding1, column_encoding1, context_encoding_row1, row_encoding1)
        res2 = self.calculate_similarity(context_encoding2, column_encoding2, context_encoding_row2, row_encoding2)

        similarity = torch.nn.functional.cosine_similarity(res1, res2)

        # return np.dot(res1.cpu().numpy(), res2.cpu().numpy())

        return similarity
    
    def calculate_similarity_cls(self, context_encoding1, column_encoding1, context_encoding_row1, row_encoding1):
        column_guided_cls = context_encoding1[:, 0]
        row_guided_cls=context_encoding_row1[:, 0]

        log_pooling_sum=torch.cat([column_guided_cls,row_guided_cls],dim=1)

        return log_pooling_sum
    
    def get_similarity_cls(self, context_encoding1, column_encoding1, context_encoding_row1, row_encoding1, context_encoding2, column_encoding2, context_encoding_row2, row_encoding2):
        
        res1 = self.calculate_similarity_cls(context_encoding1, column_encoding1, context_encoding_row1, row_encoding1)
        res2 = self.calculate_similarity_cls(context_encoding2, column_encoding2, context_encoding_row2, row_encoding2)
