...
class VerticalAttentionTableBert(VanillaTableBert):
    ...
    def encode_query(
        ...
        return context_encoding
# ------------------------------------------------------------------------------------------------------------------------------------
    # def encode_contexts(self, contexts: List[List[str]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    #     """
    #     Converts raw context lists into input tensors for the model.
    #     """
    #     # Tokenize the contexts (implement this with your tokenizer)
    #     encoded_inputs = self.tokenizer.tokenize(
    #         contexts,
    #         padding=True,
    #         truncation=True,
    #         return_tensors="pt"
    ...
    #     sequence_mask = encoded_inputs['attention_mask']

    #     return input_ids, segment_ids, sequence_mask
    def encode_contexts(self, contexts: List[List[str]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Converts raw context lists into input tensors for the model using token-level processing.
        """
        ...
        for context in contexts:
            ...
            # print(context)
            # joined_context = " ".join(context[0][1])

            # Tokenize the context
            # tokens = self.tokenizer.convert_tokens_to_ids(joined_context)

            # Add special tokens [CLS] and [SEP]
            tokens = ['[CLS]'] + context[0][1]
            print(tokens)
            # Convert tokens to IDs
            input_ids = self.tokenizer.convert_tokens_to_ids(tokens)

            # Create segment IDs (all 0s, assuming single sequence input)
        ...
        all_input_ids = [ids for ids in all_input_ids]
        all_segment_ids = [sids + [0] * (max_len - len(sids)) for sids in all_segment_ids]
        ...
        return input_ids_tensor, segment_ids_tensor, attention_mask_tensor

    def to_tensor_dict_our(self, contexts: List[List[str]], tables: List[Table] = None) -> Tuple[Dict, List, Dict]:
        """
        Converts the input contexts into tensors that are used as input to the model.
        Since we are no longer using tables, we only process the contexts.
        """
        # Process the contexts and create tensors directly
        input_ids, segment_ids, sequence_mask = self.encode_contexts(contexts)
        print("after converting tokens to ids")
        print(input_ids)

        # Create the tensor dictionary with the processed context information
        tensor_dict = {
            'input_ids': input_ids,
        ...
        # Return empty lists/dictionaries for unused variables
        return tensor_dict, [], {}


    def forward_query_our(
        self,
        ...
        **kwargs
        ...
        if isinstance(bert_output, tuple) or isinstance(bert_output, list):
            bert_output = bert_output[0]

        cls_representation = bert_output[:, :1, :]
        cls_representation = cls_representation.unsqueeze(2)

        return cls_representation

    def encode_query_our(
            self,
            contexts: List[List[str]],
            tables: List[Table],  # This argument is not needed anymore but kept for interface consistency
            return_bert_encoding: bool = False
    ) -> Tuple[torch.Tensor, torch.Tensor, Dict, torch.Tensor, torch.Tensor, Dict]:
        """
        This function handles the transformation of raw query contexts into embeddings.
        """
        assert return_bert_encoding is False, 'VerticalTableBert does not support return_bert_encoding=True'

        # Since we only care about contexts and not tables anymore, pass only contexts.
        tensor_dict, instances, tensor_dict_col = self.to_tensor_dict_our(contexts, tables=None)
        ...
        # Now we return only the necessary parts
        # print(tensor_dict)
        context_encoding = self.forward_query_our(**tensor_dict)
        # print(context_encoding)
        return context_encoding